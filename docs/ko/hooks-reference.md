...
- `.gitattributes`에서 `binary`, `-text`, `eol=crlf`로 지정된 파일도 제외
...
**`.gitattributes`를 통한 파일별 제외:**

줄 끝 또는 trailing whitespace를 그대로 유지해야 하는 파일(HTTP 픽스처,
골든 테스트 데이터, Windows 배치 스크립트 등)에 대해 검사를 건너뛰려면
`.gitattributes`에 표시하세요:

```gitattributes
# HTTP 요청/응답 픽스처는 CRLF 줄 끝이 필요
*.http        text eol=crlf
testdata/**   text eol=crlf

# 바이너리 블롭은 텍스트 검사 전혀 안 함
*.png         binary
*.bin         -text
```

`binary`, `-text`, `eol=crlf` 중 하나에 해당하는 파일은 이 훅에서 건너뜁니다.
