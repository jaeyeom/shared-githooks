...
| `check-whitespace.sh` | `git diff-index --check`로 trailing whitespace 및 혼합 줄 끝 검출 | 언어 포매터가 처리하는 파일 (*.go, *.py, *.proto, *.bzl, BUILD*), 또는 `.gitattributes`에서 `binary` / `-text` / `eol=crlf`로 표시된 파일 |
